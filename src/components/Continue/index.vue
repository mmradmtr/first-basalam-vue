<template>
  <div class="body">
    <div class="content--second">
      <div class="free--info">
        <span>هورا ارسال از این غرفه برای شما <span style="font-weight: bold">رایگان شد</span></span>
      </div>

      <div class="sale--code">
        <span class="sale--code--span"><img src="@/assets/Images/Continue/ticket.svg" alt="">ثبت کد تخفیف غرفه</span>
      </div>

      <div class="content--third">
        <div class="continue--buy">
          <span class="continue--buy--span">ادامه خرید فقط از این غرفه</span>
        </div>
        <div class="content--third--tools">
          <div class="content--third--2product">
            <span>جمع مبلغ برای <span>2</span> کالا</span>
          </div>
          <div class="content--third--price">
            <span class="text-bold">۷۵۰۰۰ تومان</span>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  // name: "Content"
  props: {
    Products: {
      type: Object,
      required: true
    }
  }
}
</script>

<style lang="scss" src="@/components/Continue/Continue.scss" scoped/>